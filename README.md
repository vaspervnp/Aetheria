...
- **Enemies & combat** — patrolling and flying corrupted algorithms plus a
  guardian, damaged by Spark's pulse attack and dash.
- **Testable core** — all simulation logic (physics, collision, noise, world
  graph, abilities, audio synthesis) is isolated from rendering so it can be
  exercised head-less by an xUnit suite and a `--smoke` simulation harness.
