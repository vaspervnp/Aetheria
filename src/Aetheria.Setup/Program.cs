...
{
    private const string AppName = "Aetheria";
    private const string AppId = "Aetheria";                       // κλειδί μητρώου / uninstall id
    private const string LinuxPackage = "aetheria";               // όνομα πακέτου & .desktop στο Linux
    private const string Publisher = "Vassilis Perantzakis";
    private const string Comment = "Descend the Bio-Mechanical Abyss — a procedurally generated Metroidvania";
    private const string DefaultSource = @"C:\Deploy\Aetheria";
    private const string FallbackVersion = "1.0";
    ...
    {
        public string ExeName => Windows ? "Aetheria.exe" : "Aetheria";
    }

    // Raylib-cs 8.0.0 ships native binaries for win-x64, linux-x64 and macOS only — there is no
    // linux-arm/arm64 native, so an ARM Linux build cannot load Raylib and is not offered.
    ...
    /// <summary>The native Raylib library that must ride along, per platform (a Linux payload
    /// without it would launch and immediately die with a DllNotFoundException).</summary>
    private static string NativeLib(Target t) => t.Windows ? "raylib.dll" : "libraylib.so";
    ...
    private static int Main(string[] args)
    {
        ...
        foreach (var target in wanted)
        {
            ...
            if (!File.Exists(Path.Combine(payload, NativeLib(target)))
                && !Directory.EnumerateFiles(payload, NativeLib(target), SearchOption.AllDirectories).Any())
            {
                Console.WriteLine($"[skip] {target.ArchLabel}: {NativeLib(target)} missing — the game can't run without Raylib, so no installer is built");
                continue;
            }
    ...
    private static void Usage() => Console.WriteLine($"""
        {AppName} installer builder
          ...
          dotnet run --project src/Aetheria.Setup -- [options]
          ...
          --targets LIST  comma separated: win-x64,linux-x64,linux-arm64 (default: all present)
    ...
    {
        string exe = Path.Combine(source, "WinX64", "Aetheria.exe");
    ...
    private static string BuildWindowsInstaller(string payloadDir, string outputDir, string version, Target target)
    {
        ...
        string staging = Path.Combine(Path.GetTempPath(), "aetheria-setup-" + Guid.NewGuid().ToString("N")[..8]);
