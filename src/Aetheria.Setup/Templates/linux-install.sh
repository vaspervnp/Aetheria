...
# Δημιουργήθηκε από το Aetheria.Setup. Το tar.gz είναι κολλημένο στο τέλος αυτού
...
Saved games live in \$XDG_CONFIG_HOME/Aetheria (~/.config/Aetheria) and are never
...
echo "  unpacking..."
mkdir -p "$INSTALL_DIR"
PAYLOAD_LINE=$(awk '/^__AETHERIA_PAYLOAD_BELOW__$/ { print NR + 1; exit 0; }' "$0")
...
Keywords=metroidvania;platformer;action;adventure;abyss;
StartupNotify=true
...
echo ""
echo "Note: Aetheria draws with OpenGL through X11 - a normal desktop install has"
echo "      everything it needs (mesa/libGL, libX11). No audio libraries required."
exit 0

__AETHERIA_PAYLOAD_BELOW__
